      ...
      context('And I enter on my account', () => {
        ...
        context('And I go to the profile fill page', () => {
          ...
          context('And I fill the fields correctly', () => {
            beforeEach(() => {
              cy.get(':nth-child(7) > .custom-control > .checkbox-outline__label')
                .click()
              cy.get(':nth-child(7) > .custom-control > .checkbox-outline__label')
                .click()
              cy.get('.css-xb97g8')
              ...
              cy.get('#years_experience')
                .select(2)
                .should('contain', 'Até um ano')
              cy.get("#preferences\\\.job_search")
                .select(2)
                .should('contain', 'Estou empregado mas busco novos desafios')
              cy.get('.react-tel-input > .form-control')
                .clear()
                .type('85999999999')
              cy.get("#address\\\.city")
                .clear()
              ...
              cy.get(':nth-child(3) > :nth-child(6)')
                .click()
              cy.get(':nth-child(3) > :nth-child(7)')
                .click()
              cy.get('#communities-0-relation')
                .select(1)
                .should('contain', 'Entusiasta')
              cy.get('#communities-1-relation')
                .select(1)
                .should('contain', 'Entusiasta')
              cy.get('.col-lg-12 > .react-select > .css-yk16xz-control > .css-13tza3w')
                .type('mulher {enter}')
              cy.get('#race')
                .select(4)
                .should('contain', 'Pessoa Parda')
              cy.get('#gender')
                .select(2)
                .should('contain', 'Mulher')
              cy.get('#sexual_orientation')
                .select(1)
                .should('contain', 'Heterossexual')
              cy.get("#disabilities\\\.type")
                .select(1)
                .should('contain', 'Nenhuma deficiência')
            ...
            it('Then I finish filling my profile correctly', () => {
              cy.url()
                .should('be.equal', 'https://beta.coodesh.com/onboarding/developer/curriculum')
              cy.get('.styles_skillsPrint___oa3j > :nth-child(1)')
                .should('be.visible')
            })
          })
        })
