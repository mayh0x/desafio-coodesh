      ...
      context('When I enter the login page', () => {
        ...
        context('And I login in an account', () => {
          ...
          context('And I go to the companies page', () => {
            beforeEach(() => {
              cy.get('#header-nav-bar > .u-header__navbar-nav > :nth-child(1) > .nav-link').click()
              cy.url().should('be.equal', 'https://beta.coodesh.com/vagas')
            })
            it('Then I search for a company and see the results', () => {
              cy.get('.col-lg-7 > .input-group > .form-control').type('Teste')
              cy.get('.align-self-lg-end > .btn-block').click()
              cy.get(':nth-child(1) > .card > .p-3').should('contain', 'Teste')
            })
          })
        })
      })
